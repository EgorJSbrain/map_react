...
export const Map = ({selectPosition, homePosition, points, handleSetPosition}: MapProps) => {
    ...
    <MapContainer center={POSITION_CENTER} zoom={8}>
      ...
      {points &&
        points.map((point) => {
          ...
          return (
            <Marker key={point.id} position={pointLocation} icon={IconsMap.pointIcon}>
              <MarkerPopup selectPosition={point.place} />
            </Marker>
          );
        })}
      <MapEvents
